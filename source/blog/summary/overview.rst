...
| このような場合には、機械学習が活躍します。機械学習により、ユーザの購入履歴からユーザの購買行動のパターンを見出し、ユーザが購入してくれる可能性が高いものを推薦することができます。
| 機械学習を使うべき理由を明確にする過程で様々な調査をするはずなので、機械学習に関するノウハウも溜まっていきます。
...
**********************************************************************************
| 機械学習の作業にかかる時間の大半はデータの準備であると言われますが、データの量と質は精度を左右する要因の1つです。
| 例えば、クレジットカードの不正検知を教師あり学習の分類問題として対処する場合を考えます。
| この場合、不正利用あり/なし (正解データと不正解データ) の両方を用意する必要があります。
...
| なお、必要となるデータ数は場合によって異なるため、テストデータに適用した場合の精度から判断する必要があります。
...
| そのような事態をなくすために、不正利用を検知した場合は登録されている電話番号に確認用の自動電話を掛けるなど機械学習モデルの判断に歯止めを効かせる仕組みを検討する必要があります。
