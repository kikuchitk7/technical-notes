...
自身のビジネスにおける問題・課題から入る
***********************************************
| 方々で言われていることですが、自身の「ビジネスにおける問題・課題」から入ることが重要です。
| すなわち、機械学習を利用することはビジネスにおいて価値を出すための手段であり、機械学習の導入を目的化しないということです。
| 例えば、「機械学習を使ってとりあえず何かをやりたい！」という思いから出発してしまうパターンです。機械学習は技術的なトレンドになっていますので、試してみたい・世の中に遅れを取りたくないなどの気持ちはわかります。
| 機械学習に限りませんが、手段と目的が逆転した状態で開発を進めてしまうと使われないシステムや機能が作られてしまいがちです。技術検証やPoC (Proof of Consent; 概念実証) を実施するにしても、背景にビジネス課題があって、それを確認するために実施するはずです。

機械学習を使って対処することが最善策なのかを考える
****************************************************
| 
| 機械学習を使ったシステムを本番環境で稼働させる場合は予測精度の監視やデータの傾向が変わった際の再学習も必要であるため、維持運用は大変 (手がかかる)。
| 機械学習モデルの予測精度は一般的に100%になることはなく、間違った判断をすることもある。これらを踏まえると、ビジネス課題が機械学習を使わずに対処できる問題・課題であるならば、使わずに対処した方が良いと考えられる。
