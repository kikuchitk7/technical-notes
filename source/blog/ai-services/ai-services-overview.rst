...
| パターン②のデメリットは、AutoML 機能を使える対象が限定されることです。
| 例えば、サブスクリプションサービスを提供している企業が「ユーザの離反予測を行う機械学習モデルを構築したい」という場合です。
| AI サービスの AutoML 機能では提供されていないユースケースであるため、対応することができません。この場合は、Amazon SageMaker を利用して構築する必要があります。
