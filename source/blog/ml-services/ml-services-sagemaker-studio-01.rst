...
Amazon SageMaker Notebooks は、JypyterLab をベースとしており、

利用者がノートブックインスタンスを作成する必要がありません。
インスタンスタイプやノートブックインスタンスにアタッチするディスクを意識する必要がありません。
これらは AWS が管理してくれます。
利用者はノートブックインスタンスの作成や維持・運用の作業から解放されます。
...
.. Amazon SageMaker Studio により、機械学習の開発における UX を変革
.. ノートブックインスタンスとの違いを見てみるとわかりやすい
.. 　　JupyterLab ベースであることは同一
.. 　　Studio は AWS SSO が使える。IAM ユーザを持たないユーザでも利用が可能。
.. 　マネジメントコンソールのあちこちに飛ばずに多くが Studio の画面内で作業が完結する


.. Amazon SageMaker Studio がもたらすもの
.. -------------------------------------------------
