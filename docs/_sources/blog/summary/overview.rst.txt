...
機械学習を活用する際の注意事項
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
本連載ではビジネスに機械学習を活用することをテーマとしていますが、その前提として少なくとも下記の2点に気を付けていただきたいと考えています。
...
機械学習を使って対処することが最善策なのかを考える
****************************************************
| 端的にいうと、機械学習を使わずに対処可能な場合や機械学習任せにするとリスクがある場合は使わないということです。
| 例えば、システムの障害とその復旧を例に考えてみます。
| 特定のエラーメッセージの検知とその復旧に必要な処理がいくつかの条件の組み合わせで決まるのであれば、条件分岐で表現すれば済みます。
| また、機械学習モデルの判断1つで広範囲のユーザに影響範囲を及ぼす、もしくは、人命に関わるような場合においても判断を機械学習モデル任せにするのはリスクが高いです。参考情報程度に留めて最終判断は人間がするべきです。

| 機械学習は過去のデータから機械学習アルゴリズムに従って何らかのパターンを見出し(学習し)、将来を予測するものです。それは文字通り「予測」であり、精度は一般的に100%にはならないので、間違った判断をすることがあります。
| 判断の間違いがあることを理解した上で利用するか、許容されない場合は適用を避けるか最終的な判断を人間がすべきです。

| また、機械学習モデルの維持や運用に手間がかかることも理由の1つに挙げられます。
| 例えば、本番環境にデプロイした機械学習モデルは、定常的に予測精度を監視して、自社のサービスとして十分な精度を出し続けているかを監視する必要があります。
| 過去のデータからパターンを抽出するということは、機械学習モデルを使って予測する対象がその過去のデータに即した振る舞いをするという前提を置いています。
| 振る舞いの傾向が変わるなどした場合 (例えば、利用者の購買行動の変化など) は、新たなデータを使って機械学習モデルを再学習させる必要があります。
