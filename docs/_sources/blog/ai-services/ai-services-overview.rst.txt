...
| サービスによって自動化範囲が多少異なりますが、多くが機械学習のワークフローのうちの「モデルの構築 (機械学習アルゴリズムの選定を含む)」「モデルの学習」「モデルの評価」のプロセスを自動化します。
...
| また、単純に利用するだけのパターン①と比べると導入までにかかる時間はかかるものの、多くの処理が自動化されているため、Amazon SageMaker 等を使ってゼロから構築する場合と比べると迅速にビジネスに機械学習を導入することができます。
