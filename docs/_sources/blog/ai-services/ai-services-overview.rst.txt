...
| また、単純に利用するだけのパターン①と比べると導入までに多少時間はかかるものの、多くの処理が自動化されているため、Amazon SageMaker 等を使ってゼロから構築する場合と比べると迅速にビジネスに機械学習を導入することができます。

| パターン②のデメリットは、AutoML 機能を使って自動構築できる機械学習モデルが限定されることです。
| 例えば、サブスクリプションサービスを提供している企業が「ユーザの離反予測を行う機械学習モデルを構築したい」という場合を考えます。
| 現在は、ユーザの離反予測を行うための機械学習モデルを構築する機能が提供されていないため、AI サービスの AutoML 機能では対応することができません。
| この場合は、Amazon SageMaker を利用して構築する必要があります。
